...
@WebMvcTest(controllers = UCSBDiningMenuItemsController.class)
public class UCSBDiningMenuItemsControllerTests extends ControllerTestCase {
  ...
  public void anonymous_caller_can_use_the_proxy_and_headers_are_forwarded() throws Exception {
    when(diningProxyService.proxyGet(
            eq("/dining/menu/v1/2024-08-16/carrillo/lunch"), eq("key123"), eq("1.0")))
        .thenReturn(ResponseEntity.status(HttpStatus.OK).body("[{\"name\":\"Entree\"}]"));

    MvcResult response =
        mockMvc
            .perform(
                get("/dining/menu/v1/2024-08-16/carrillo/lunch")
    ...
    verify(diningProxyService, times(1))
        .proxyGet("/dining/menu/v1/2024-08-16/carrillo/lunch", "key123", "1.0");
