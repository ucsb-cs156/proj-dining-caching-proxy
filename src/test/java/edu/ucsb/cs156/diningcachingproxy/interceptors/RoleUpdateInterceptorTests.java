...
import edu.ucsb.cs156.diningcachingproxy.controller.DummyController;
...
public class RoleUpdateInterceptorTests {
  ...
  public void regular_user_has_only_user_role() throws Exception {
    ...
    when(hostManagerRepository.existsByEmail("user@ucsb.edu")).thenReturn(false);
  ...
  @Test
  public void hostManager_in_db_gets_hostManager_role() throws Exception {
    when(adminRepository.existsByEmail("user@ucsb.edu")).thenReturn(false);
    when(hostManagerRepository.existsByEmail("user@ucsb.edu")).thenReturn(true);
            ...
            get("/dummycontroller/interceptorTest")
                .with(
                    oidcLogin()
                        .userInfoToken(t -> t.email("user@ucsb.edu"))
                        .authorities(new SimpleGrantedAuthority("ROLE_USER"))))
        .andExpect(authenticated().withRoles("USER", "HOSTMANAGER"));
  }
  ...
  @Test
  public void hostManager_role_removed_when_user_loses_hostManager_status() throws Exception {
    when(adminRepository.existsByEmail("user@ucsb.edu")).thenReturn(false);
    when(hostManagerRepository.existsByEmail("user@ucsb.edu")).thenReturn(false);
            ...
            get("/dummycontroller/interceptorTest")
                .with(
                    oidcLogin()
                        ...
                        .authorities(
                            new SimpleGrantedAuthority("ROLE_USER"),
                            new SimpleGrantedAuthority("ROLE_HOSTMANAGER"))))
        .andExpect(authenticated().withRoles("USER"));
  }
