 ...
 * successfully and that at least one known controller tag (User information) is rendered on the
 * page. This is meant to catch cases where the Swagger page fails to load entirely, without
...
public class SwaggerWebIT extends WebTestCase {
  ...
  public void swagger_ui_page_loads_successfully() throws Exception {
    ...
    // either a successful render (the "User information" tag) or a failed one (the error
    ...
    Locator userInfoTag =
        page.locator("span")
            .filter(new Locator.FilterOptions().setHasText("User information"))
            .first();
    ...
    assertThat(userInfoTag).isVisible();
  }
}
